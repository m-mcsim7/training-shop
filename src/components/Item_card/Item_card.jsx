  ...
  return (
    <div className="sort__items">
      {genderItems.map((card, index) => (
        ...
        >
           {card[0].discount && <span className="item__card-sale">{card[0].discount}</span>}
          ...
          <div className="item__card-suptitle">
            <div className="item__card-price">$ {card[0].price} 
            {card[0].discount && <span className="item__card-price-sale">$ {(Math.round((card[0].price - card[0].price / 100 * (card[0].discount.slice(1, -1)))*100) / 100)}</span>}
            </div>
