...
function Item(props) {
  ...
  if (!isEmpty(productsAPi)) {
    arr = [...productsAPi[props.product_type]];
  }
  if (true) {
    for (let i of arr) {
      if (i.id === cardID) {
  ...
  let arrColor = [];
  let strColor = [];
  let arrColorIncludes = [];
  if (!isEmpty(productsAPi)) {
    arrColor = card.images.map((item) => item.color);
    strColor = new Set(arrColor);
  ...
  let colorArrIncludes = [];

  if (!isEmpty(productsAPi)) {
    colorArrIncludes = [card.images[0]];
